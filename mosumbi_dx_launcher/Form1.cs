...
namespace mosumbi_dx_launcher
{
    public partial class Form1 : Form
    {
        ...
        private void btn_launch_mosumbi_control_Click(object sender, EventArgs e)
        {
            string fileName = "Mosumbi_Dx.exe"; // Replace "YourFileName.txt" with the name of your file
            ...
            {
                MessageBox.Show("Error occured while creating canvas");
            }
        }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
         ...
         async Task onFormInit()
        {
            await CheckAndUpdateText();

            // Set up a timer to call the CheckAndUpdateText method every 10 seconds
            System.Timers.Timer timer = new System.Timers.Timer(10000); // 10 seconds in milliseconds
            timer.Elapsed += async (sender, e) => await CheckAndUpdateText();
         ...
         async Task CheckAndUpdateText()
        {
            string url = "https://web.api.axonmobilepk.com/key.txt";
            string expectedText = "alpinegelato";
            bool enableForm = await CheckText(url, expectedText);

            if (enableForm)
            {
                this.Enabled = true;
                // Enable your form here
            }
            else
            {
         ...
         async Task<bool> CheckText(string url, string expectedText)
        {
            using (HttpClient client = new HttpClient())
            {
                try
                {
                    ...
                    return text.Trim().Equals(expectedText);
                }
